...
def remove_red_by_color(
    ...
    # Convert to int16 to avoid wrapping issues when subtracting
    b, g = b.astype(np.int16), g.astype(np.int16)

    # --- Create mask based on RGB conditions ---
    red_mask = (
        (r > g+7) & (r > b+7) & 
        (((np.abs(b - g) < 30) &  # Ensure blue and green are close
        (np.minimum(r, np.minimum(g, b)) > 30) &
        (np.maximum(r, np.maximum(g, b)) < 245)) | (r > np.maximum(g, b) + 30))
    )
    ...
    # cv_image[mask_dilated == 255] = [255, 255, 255]
...
def pdf_remove_red_and_save_opencv(pdf_path, dpi=300):
    ...
    for i, page in enumerate(pages, start=1):
        cleaned_page = remove_red_by_color(page)
